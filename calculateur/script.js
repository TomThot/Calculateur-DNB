...
if (moyenneOption !== "") {                                               // si la moyenne à l'année n'est pas vide
  ...
  const moyenneGlobale = lignesAvecMoyenne > 0 ? (sommeGlobale / lignesAvecMoyenne).toFixed(2) : "–"; //Si au moins une ligne a une moyenne (lignesAvecMoyenne > 0), on calcule la moyenne générale :
  ...
  const rows = table.querySelectorAll("tbody tr");                                      //récupère toutes ses lignes (<tr>) dans le corps (<tbody>).
        ...
        /// Teste si les valeur entrées pour les notes finales sont indérieur à20///
